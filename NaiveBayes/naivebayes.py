...
def NB_model_confound(X_train, y_train, X_test, y_test):
    model = GaussianNB()
...
def NB_model_nonconfound(X_train, y_train, X_test, y_test):
    model = GaussianNB()
    model.fit(X_train, y_train)
    y_pred = model.predict(X_test)
...
def main():
    ...
    # Train and display results for non-confound data using Naive Bayes
    print("Naive Bayes Results for Non-Confound Data:")
    nb_model_nonconfound = NB_model_nonconfound(X_train_nonconfound, y_train_nonconfound, X_test_nonconfound, y_test_nonconfound)

    # Train and display results for confound data using Naive Bayes
    print("Naive Bayes Results for Confound Data:")
    nb_model_confound = NB_model_confound(X_train_confound, y_train_confound, X_test_confound, y_test_confound)
