...
def main():
    top29Subjects = [173B, 176, 177, 183, 187B, 195B, 200, 207, 208]
    ...
    for subjID in top29Subjects:
        for task in taskType:
            for run in num_runs:
                ...
                print(f"Extracting betas for Subject {subjID}, Task {task}, Run {run}")
                beta_weights = extract_beta_weights(run, subject_id=subjID, task_type=task)
                print(f"DONE for Subject {subjID}, Task {task}, Run {run}")
